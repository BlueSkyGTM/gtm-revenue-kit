...
account's `context/scoring-model.md`, per `docs/isolation.md`.
...
5. **Score and route.** Run `skills/icp-scoring/SKILL.md`. Tier bands (set in the account's
   `scoring-model.md`) decide the treatment: top-tier visits get a human first touch fast;
   mid-tier visits enter a signal-specific sequence via `skills/signal-to-sequence/SKILL.md`.
...
them something about their own business — the PVP standard in `docs/standards.md` applies in
...
- The campaign gates and funnel diagnostics in `docs/standards.md` govern launch and kill
