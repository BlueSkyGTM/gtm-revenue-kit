...
3. **Enrich and score.** Fill the preference attributes and rank — the logic of
   `skills/icp-scoring/SKILL.md` with role-fit dimensions; weights and shortlist cutoffs
...
`docs/standards.md`: stripped of the ask, the message should still tell the candidate
...
- Reply and screen-booking rates per role and per message variant — read with the funnel
  diagnostics from `docs/standards.md`: low reply is the message, replies-but-no-screens is
