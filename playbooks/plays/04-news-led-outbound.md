...
(the standard `docs/standards.md` requires) that the prospect themselves would agree is real:
...
6. **Score and route.** Run `skills/icp-scoring/SKILL.md`; the event's point value and the
...
before: the PVP standard from `docs/standards.md`, with the event date as the built-in
datable "why now."
...
- Reply and meeting rates versus the account's cold baseline — the timing advantage should
  show up plainly, per the benchmarks in `docs/standards.md`
...
- **Extraction quality is unproven.** Sending outreach based on a misread article — wrong
  company, wrong event — fails the verify-specifics standard in `docs/standards.md` in the
...
- **Sensitive events.** Some newsworthy events (layoffs, disasters, litigation) create real
  ...
  `playbooks/plays/12-competitor-crisis-response.md`.
