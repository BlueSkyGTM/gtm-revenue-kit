...
2. **Enrich to scoreable.** Fill the attributes the account's `scoring-model.md` actually
   scores on — nothing more. Enrichment coverage below the campaign gate in
   `docs/standards.md` means the scores are fiction; fix coverage before tiering.
3. **Score and tier.** Run `skills/icp-scoring/SKILL.md` across the map. Band boundaries
...
"why now is datable" standard in `docs/standards.md` before any first touch. Tier rank is a
...
- Map health: enrichment coverage and staleness (time since last refresh), held against the
  gates in `docs/standards.md`
...
- **To dodge the segment-purity rule.** Tiers are budget bands, not audiences. Blending
  tiers into one campaign to hit the audience-size gate violates the segmentation standard
