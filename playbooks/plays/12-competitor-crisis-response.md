...
2. **Pre-wire the customer identification.** Keep the technographic mapping of
   ...
   `playbooks/plays/11-renewal-window-targeting.md`), so the affected segment is one
   filter away, not one project away.
3. **On verified trigger: cut the segment.** Filter the TAM to companies on the affected
   competitor, run `skills/icp-scoring/SKILL.md`, apply the account's `optouts.md`, and
...
5. **Ground game: executive 1:1 outreach.** Executives (not SDRs — see
   `playbooks/plays/13-executive-channel-outreach.md`) reach the segment's
...
6. **Route conversations into the competitive motion.** Replies flow into
   `playbooks/competitor-switch.md` (Scenario B, active evaluation) with battlecards
...
standard from `docs/standards.md`. The "why now" is datable but deliberately unstated;
...
- Reply, meeting, and evaluation-entry rates for the affected segment versus the
  account's baseline competitive outreach, read against the benchmarks in
  `docs/standards.md`
