...
`motions/workflows/campaign-build.md` starts from a defined segment and a signal: an audience already
...
Use `motions/workflows/campaign-build.md` instead when a specific signal already defines the audience.
...
2. **The other half each need a bespoke source**, and finding it is genuine research: deep
   ...
   `motions/workflows/enrichment-techniques.md` — read that before concluding a market can't be mapped.
...
2. **Answer each question with the cheapest tool that can answer it** — free formatting
   ...
   is in `motions/workflows/enrichment-techniques.md`.
...
`motions/workflows/enrichment.md` owns the enrichment waterfall and quality gates; this stage is about
...
API can do both (see the minimal-stack pattern in `motions/workflows/enrichment-techniques.md`).
Unvalidated emails are not contacts; they are future bounces, and the bounce ceiling in
`motions/playbooks/deliverability-and-warmup.md` is easy to blow with one unvalidated batch.
...
cold-call channel (`motions/channels/cold-calls.md`).
...
- Once yield is known, infrastructure sizing becomes arithmetic: replies needed → sends
  needed → mailboxes and domains needed (`motions/playbooks/deliverability-and-warmup.md`).

**Doctrine note (open test: `experiments/001-first-touch-formula.md`).** The fixed-slot template above and the
kit's PVP standard (`foundations/pvp.md`) are **parallel copy instruments, and the choice
between them is queued as a test (`experiments/001`), not resolved.** By PVP's own test the
...
Every pattern must survive the verification standard in `foundations/pvp.md`: a confidently
...
The durable home for infrastructure decisions is `motions/playbooks/deliverability-and-warmup.md`
(domains, warmup, the free/safe/scaled tradeoff, decisions already made) and
`motions/workflows/enrichment.md` (the standing setup rules applied during list build). Read those;
...
- Only validated emails are enrolled — "valid" beats "catch-all," and the account's bounce
  ceiling (with the defaults and rationale in `motions/playbooks/deliverability-and-warmup.md`)
  gates every batch
...
- `motions/workflows/campaign-build.md` — the signal-led motion; also the QA, launch, and
  iterate/retire machinery that this workflow's output feeds into
- `motions/workflows/enrichment.md` — the enrichment waterfall and data-quality gates for Stage 3
- `motions/workflows/enrichment-techniques.md` — cost model, API/webhook patterns, and the scraping
  patterns Stage 1 depends on
- `motions/playbooks/deliverability-and-warmup.md` — the infrastructure under Stage 6
- `motions/channels/` — channel strategy for what runs on the finished list
- `foundations/pvp.md` — PVP, why-now, and verification; the account's `scoring-model.md` — tiers and benchmarks
