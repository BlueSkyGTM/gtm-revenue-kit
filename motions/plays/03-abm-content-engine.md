...
the account also runs buyer-group mapping (`motions/dormant/buyer-group-mapping.md`), that
...
sourced materials — the "verify specifics" standard in `foundations/pvp.md` applies to
...
1. **Codify the standing inputs.** The account's `context/positioning.md`,
   ...
   `motions/playbooks/impact-positioning.md` — the engine renders *from* the house, never freelances.
...
4. **Generate against templates.** Each content type has a fixed structure; the pipeline
   ...
   marker — the `[PROOF GAP]` convention from `motions/playbooks/impact-positioning.md`.
...
`foundations/pvp.md` transfers cleanly: strip the product and the asset should still teach
...
- **Positioning unsettled.** If `context/positioning.md` and the messaging house are not
  stable, the engine mass-produces drift. Run `motions/playbooks/impact-positioning.md` to
  completion first.
