...
6. **Receive introductions as Tier 1.** Every introduction gets researched
   (`motions/skills/account-research/SKILL.md`) and handled with bespoke care — a fumbled
...
from `foundations/pvp.md` like any first touch — the introduction bought attention, not a
free pass to pitch. The datable "why now" is whatever made the partner think of them, and
the follow-up sequence discipline (`motions/skills/signal-to-sequence/SKILL.md`) applies from there.
...
- Conversion: introductions → conversations → pipeline → closed, priced against fees
  ...
  below the account's campaign-gate thresholds (`scoring-model.md` §8), deliberately
