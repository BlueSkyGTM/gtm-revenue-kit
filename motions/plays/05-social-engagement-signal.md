...
5. **Suppress and score.** Check the CRM and the account's `optouts.md`, then run
   `motions/skills/icp-scoring/SKILL.md` — engagement is one input among the account's other
   signals, and the combination bonuses live in the `scoring-model.md`.
6. **Route to a topic-matched sequence.** Via `motions/skills/signal-to-sequence/SKILL.md`, with
...
the PVP standard (`foundations/pvp.md`). The datable "why now" is the observable
