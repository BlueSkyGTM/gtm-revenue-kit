...
3. **Qualify the destination.** Score the new company against
   `accounts/<slug>/context/icp-definition.md` via `motions/skills/icp-scoring/SKILL.md`. Fit
...
`foundations/pvp.md` holds — the note should be worth receiving from someone they never buy
...
- Volumes are watchlist-sized, so read rates with the same small-sample caution as
  `motions/plays/06-one-to-one-abm.md`; the account's campaign gates (`scoring-model.md` §8) apply
