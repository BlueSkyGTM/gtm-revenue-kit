...
motions like `motions/plays/12-competitor-crisis-response.md` or any sequence built by
`motions/skills/signal-to-sequence/SKILL.md` where the account chooses the executive channel. Two
...
6. **Hand off live conversations.** The operator triages; the executive (or an AE, if
   ...
   account's standard motion via `motions/playbooks/new-signal-response.md` conventions.
...
then it must satisfy the PVP standard (`foundations/pvp.md`) like any first touch: the
