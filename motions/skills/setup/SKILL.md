...
```
Read motions/skills/setup/SKILL.md and set up an account for [domain.com] as [slug]
```
...
4. **Fill the identity block** at the top of `accounts/<slug>/ACCOUNT.md`: slug, tier
   (a deliberate operator choice), status `scaffolded`, operator, principal.

Everything below writes inside `accounts/<slug>/`. Nothing this skill does touches core or
another account (`foundations/principles.md`).
...
account without this file cannot be scored (`foundations/principles.md` §3).

Fill with:
- §1 dimension weights — start from the template's restored upstream defaults unless the research
...
You can start using the repo right now:
  ...
  Read motions/skills/account-research/SKILL.md and research [example account from their ICP]
