# motions/skills/ — the execution contract
...
- **Inputs discipline:** each `SKILL.md` has an `## Inputs` block naming exactly the files
  it reads — never bulk-load `context/` or `outputs/` (`foundations/principles.md`).
- **Values live in the account:** every number a skill needs comes from
  `accounts/<slug>/context/scoring-model.md` (`foundations/principles.md` §3).
