...
- The account's addressable market is small: the volume math in
  `motions/channels/cold-email.md` doesn't have room to run
...
Start from the account's mapped TAM (`motions/tam/skeleton.md`) and cut it into **small,
...
Every specific must survive the verification standard in `foundations/pvp.md` — in this
...
4. **Replies are worked like the high-value conversations they are** — same-day response,
   phone where possible (`motions/channels/cold-calls.md`)
...
- `motions/channels/linkedin-abm.md` — the same economics with LinkedIn as the spine;
  the recognition-asset architecture is a natural micro-list ask
- `motions/channels/cold-email.md` — the opposite pole; read both before choosing
- `motions/tam/skeleton.md` — the TAM map the segments are cut from
- `motions/playbooks/deliverability-and-warmup.md` — why hand-sent mail from a real mailbox plays
  by different rules than platform volume
- `foundations/pvp.md` — verification and the "why now" standard, at their strictest here
